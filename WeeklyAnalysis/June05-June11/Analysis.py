...
DataFilter.to_excel('DataFiltered.xls')
DataFilter2.to_excel('DataFilteredDateMerchant.xls')
print('Total: ' + str(DataFilter3['Debit'].sum()))
DataFilter4.to_excel('DataFilteredMonth.xls')

sns.set_style('white')
sns.set_context('paper', font_scale = 1.5)
sns.set_color_codes("pastel")
ax = sns.countplot(x='Weekday', data=DataFilter)
ax.yaxis.set_major_locator(MaxNLocator(integer=True))
ax.set(xlabel='Weekday', ylabel='Transactions')
plt.xticks(rotation=30)
plt.tight_layout()
plt.show()
ax2 = sns.barplot(data = DataFilter, x = 'Merchant', y = 'Debit', hue='Weekday', ci = None)
...
ax3 = sns.lineplot(data = DataFilter3, x = 'Date', y = 'Debit', color = 'r')
plt.xticks(rotation=30)
plt.tight_layout()
plt.show()
